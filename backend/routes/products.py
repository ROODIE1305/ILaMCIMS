...
from backend.models.products import see_products, add_products, update_products
...
async def get_products():
    try:
        ...
        return [see_products(**product) for product in response.data]
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error fetching products: {str(e)}")

# Post method
@router.post("/", response_model=see_products)
async def post_products(new_products: add_products):
    try:
        ...
        return see_products(**response.data[0])
    except Exception as e:
...
async def put_products(product_id: int, change_product: update_products):
    try:
